...
export default function Filter() {
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:8080/edumato/restaurants/filter/${currentPage}`,requestOptions)
...
, [filter,currentPage])
  ...
  const handleLocationChange=(event)=>{
      ...
      .then((res) => {
        ...
        const SelectedCity = locations.find((city) => city.city_id == event.target.value);
        const selectedCityLocations = locations.filter(
          (city) => city.city_id == event.target.value
          
          )
            setcity( selectedCityLocations[0].name)
          console.log("cityname=",selectedCityLocations[0].name)
        }
          )
        }
// console.log("ssss",SelectedCity.city_name)
   ...
   const locationList=locations.length>0 && locations.map((item)=>
  ( <option  key={item.name} value={item.city_id} on>{item.name}</option>))
    return (
    
        <div>
            
            
               <div id="myId" className="heading-filter">{mealtype} Places in  {city}</div>
                            ...
                            <div className="col-sm-9 col-md-9 col-lg-9 scroll">
                             {
                                 ...
                                 <Link to={`/edumato/restaurantDetails/${item.name}`}><div className="Item">
                                ...
                                //  <div className="noData"> No Data Found</div>
                                 ...
                                 <div className="row ">
                                     <div className="col-sm-8 col-md-8 col-lg-8">
                                         <div className="rest-name text-center mt-5">No Data Found</div>
                                     </div>
                                 </div>
                             </div>
